...
codebuddy_hook_url = urllib.parse.urlunparse(parsed._replace(path="/codebuddy-hook"))
...
urls = [codebuddy_hook_url]
if parsed.hostname in ("127.0.0.1", "localhost"):
    try:
        with open("/etc/resolv.conf", "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith("nameserver "):
                    urls.append(urllib.parse.urlparse(codebuddy_hook_url)._replace(netloc=f"{line.split()[1]}:{parsed.port}").geturl())
                    break
    except Exception:
        pass
