...
qoder_hook_url = urllib.parse.urlunparse(parsed._replace(path="/qoder-hook"))
        ...
        with open("/etc/resolv.conf", "r", encoding="utf-8") as f:
            for line in f:
                if line.startswith("nameserver "):
                    urls.append(urllib.parse.urlparse(qoder_hook_url)._replace(netloc=f"{line.split()[1]}:{parsed.port}").geturl())
                    break
    except Exception:
        pass
